...
- **CLI backends are isolated** — every CLI backend runs with `</dev/null` and from an
  empty scratch dir (`$AGENT_CWD`). Agent backends also run with their explicit no-tools
  controls. These are load-bearing: PR diffs are untrusted, agent CLIs may read stdin and
  swallow the y/N confirmation answer, and they auto-load instruction files from the cwd.
  Do not add an agent backend unless its CLI can disable all tools. Codex, Agy and OpenCode
  are intentionally unsupported because they do not expose a suitable no-tools mode.
